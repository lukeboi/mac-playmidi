...
int main(int argc, const char *argv[]) {
    ...
    MusicSequence musicSeq;
    NewMusicSequence(&musicSeq);
    if (MusicSequenceFileLoad(musicSeq,
                              CFURLCreateFromFileSystemRepresentation(NULL, (const UInt8 *) filename, strlen(filename),
                                                                      false), kMusicSequenceFile_MIDIType, 0)) {
    ...
    if (argc > 3) {
        ...
        if (!MusicSequenceGetBeatsForSeconds(musicSeq, startSecond, &startPoint))
            MusicPlayerSetTime(mPlayer, startPoint);
        else {
    ...
    if (argc > 4) {
        ...
        UInt32 nTracks;
        if (MusicSequenceGetTrackCount(musicSeq, &nTracks)) nTracks = 1;
        MusicTimeStamp maxStopPoint;
        for (int trackNo = 0; trackNo < nTracks; trackNo++) {
            MusicTrack t;
            MusicSequenceGetIndTrack(musicSeq, trackNo, &t);
            UInt32 n = sizeof(MusicTimeStamp);
            MusicTrackGetProperty(t, kSequenceTrackProperty_TrackLength, &stopPoint, &n);
            if (!trackNo || stopPoint > maxStopPoint) maxStopPoint = stopPoint;
            else stopPoint = maxStopPoint;
        }
    }
    double stopSecs;
    MusicSequenceGetSecondsForBeats(musicSeq, stopPoint, &stopSecs);
    double playRate = 1.0;
    if (argc > 2) {
